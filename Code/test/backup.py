...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        for imu_msg in self.imu_msg_buffer:
            ...
            self.sum_angular_vel += imu_msg.angular_velocity
            self.sum_linear_acc += imu_msg.linear_acceleration
        ...
        gravity_norm = np.linalg.norm(self.gravity_imu)
        IMUState.gravity = np.array([0.0, 0.0, -gravity_norm])

        # Initialize the initial orientation, so that the estimation
        # is consistent with the inertial frame.
        self.state_server.imu_state.orientation = from_two_vectors(self.gravity_imu, -IMUState.gravity)
        # R  = to_rotation(q0_i_w)
        # self.state_server.imu_state.orientation = to_quaternion(np.transpose(R))
    ...
    def batch_imu_processing(self, time_bound):
        ...
        # del self.imu_msg_buffer[:self.used_imu_msg_cntr] ### This is different
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        Phi[0:3, 0:3] = to_rotation(self.imu_state.orientation) * R_kk_1_t
        ...
        if len(self.state_server.cam_states) > 0:
            self.state_server.state_cov[0:21 , 21:0] = Phi @ self.state_server.state_cov[0:21, 21:0]    
            self.state_server.state_cov[21:0, 0:21] = self.state_server.state_cov[21:0, 0:21]  @ np.transpose(Phi)
        ...
        # self.state_server.imu_state.timestamp = time   ## This line is not there in uoip
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        k1_v_dot = q_t @ acc + IMUState.gravity  
        k1_p_dot = v
    ...
    def state_augmentation(self, time):
        ...
        J = np.zeros((6, 21)) ##J(6,21)
        J[0:3, 0:3] = R_i_c ## (3,3) 0,0
        J[0:3, 15:18] = np.identity(3) ##(3,3) 0,15
        J[3:6, 0:3] = skew(np.matmul(R_w_i.T,t_c_i)) ##(3,3) 3,0
        J[3:6, 12:15] = np.identity(3) ##(3,3) 3,12
        J[3:6, 18:21] = np.transpose(R_w_i) ##(3,3) 3,18   ####This is identity in uoip
        ...
        state_cov[old_size:, old_size:] = J @ state_cov[:21, :21] @ np.transpose(J)
    ...
    #             tracked_feature_num= tracked_feature_num+1
    #         else: 
    ...
    def measurement_update(self, H, r): ##############?????????????????????????
        ...
        #     Q, R = np.linalg.qr(H_sparse, mode='reduced')

        #     H_temp = np.zeros()
        #     r_temp = np.zeros()

        #     matrix_power(Q.T * H, H_temp)
        #     matrix_power(Q.T * r, r_temp)

        #     H_thin = H_temp[0:21+self.state_server.cam_states.size()*6]
        ...
        K_transpose = scipy.linalg.solve(scipy.linalg.ldl(S), np.matmul(H_thin,P))  ## Remove ldl if not working
        K = K_transpose.T
        ...
        if norm(delta_x_imu[6:9]) > 0.5 or norm(delta_x_imu[12:15]) > 1.0 :   # 3 6, 3 12
            print("delta velocity: %f\n", norm(delta_x_imu[6:9]))
            print("delta position: %f\n", norm(delta_x_imu[12:15]))
        ...
        # for i in range(0,len(self.state_server.cam_states)):
        #     cam_state_iter  = cam_state_iter + 1
        #     delta_x_cam = delta_x[21+i*6:27+i*6]  #6 21+i*6
        #     dq_cam = small_angle_quaternion(delta_x_cam[0:3])
        #     cam_state_iter.second.orientation = quaternion_multiplication(
        #         dq_cam, cam_state_iter.second.orientation)
        #     cam_state_iter.second.position += delta_x_cam[-3:]  ## +3 here in uoip
        ...
        for i, (cam_id, cam_state) in enumerate(
            ...
            cam_state.position += delta_x_cam[3:]  ## Change this!!!!!!
        ...
        # Fix the covariance to be symmetric
        self.state_cov_fixed = (self.state_server.state_cov + self.state_server.state_cov.T) / 2.0
